...
export class PostsLikesQueryRepository {
  ...
  async findNewestLikes(postId: string): Promise<NewestLikesDto[]> {
    const result: PostLikeDocument[] = await this.PostLikeModel.find({postId, status: 'Like'})
      .sort({addedAt: -1})
      .limit(3)
  ...
  async findLikeByCommentAndUser(postId: string, userId: string): Promise<PostLikeDocument | null> {
