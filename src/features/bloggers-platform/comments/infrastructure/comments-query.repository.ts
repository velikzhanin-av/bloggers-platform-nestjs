...
export class CommentsQueryRepository {
  ...
  async getCommentsByPostId(
    ...
    postId: string,
    userId: string | null): Promise<PaginatedViewDto<CommentViewDto[]>> {

    const filter = { postId, deletionStatus: {$ne: DeletionStatus.PermanentDeleted} }
    ...
    const items: any = await Promise.all(
      comments.map(async (comment: CommentDocument) => {
        if (!userId) return CommentViewDto.commentMapToView(comment, LikeStatus.None)
        const like: CommentLikeDocument | null =
          await this.commentLikeRepository.findLikeByCommentAndUser(
            userId,
            comment.id.toString(),
          );
        if (!like) return CommentViewDto.commentMapToView(comment, LikeStatus.None);
